...
public class DataFilterAspect {
  ...
  @Pointcut("@annotation(com.qidiancamp.common.annotation.DataFilter)")
  ...
  private String getSQLFilter(SysUserEntity user, JoinPoint point) {
    ...
    if (roleIdList.size() > 0) {
      List<Long> userDeptIdList =
          sysRoleDeptService.queryDeptIdList(roleIdList.toArray(new Long[roleIdList.size()]));
    ...
    sqlFilter
        .append(tableAlias)
        .append("dept_id in(")
        .append(StringUtils.join(deptIdList, ","))
        .append(")");
