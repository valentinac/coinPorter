...
public class BinanceAdapters {
  ...
  public static Order adaptOrder(BinanceOrder order) {
    ...
    if (order.executedQty.signum() == 0
        || order.type.equals(com.qidiancamp.api.binance.dto.trade.OrderType.MARKET)) {
    ...
    Order result;
    if (order.type.equals(com.qidiancamp.api.binance.dto.trade.OrderType.MARKET)) {
      result =
          new MarketOrder(
              type,
              order.origQty,
              ...
              BigDecimal.ZERO,
              orderStatus);
    } else if (order.type.equals(com.qidiancamp.api.binance.dto.trade.OrderType.LIMIT)
        || order.type.equals(com.qidiancamp.api.binance.dto.trade.OrderType.LIMIT_MAKER)) {
      result =
          new LimitOrder(
              type,
              order.origQty,
